...
<script>
  ...
  $(document).ready(function () {
    ...
    function fetchStudents() {
      $.get('http://localhost:8000/api/students', function (data) {
        let rows = '';
        data.forEach(student => {
          rows += `
            <tr>
              <td>${student.id}</td>
              <td><img src="http://localhost:8000/storage/${student.photo}" style="width: 50px; height: 50px; border-radius: 50%;"></td>
